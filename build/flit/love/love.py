...
def main(proposal=None, target_dir=None):
    ...
    if token is None:
        ...
        log.info("I'll try to open github for you at the right page, otherwise please visit", GITHUB_NEW_TOKEN_URI)
        sleep(5)
    ...
    if closest:
        if proposal in closest:
            ...
            log.warn(proposal, 'name is close to the following package name:', closest)
    else:

        log.info(proposal, 'seems to have a sufficiently specific name, continuing...')
    ...
    repo, user = enable_travis(token, slug) 
    project_layout(proposal, user, repo)


# insert travis here. 

def enable_travis(token, slug):
    """
...
def project_layout(proposal, user):
